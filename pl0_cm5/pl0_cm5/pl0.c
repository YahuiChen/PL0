...
void enter(char const *const sym_id, enum object k, DataType down_type)
{ // enter object into table
    ...
    case TAG:
        ...
        type_table[type_tx].dope_vec = g_dope_vec; // g_dope_vec 的上下界在 type_expr() 中获得
        type_table[type_tx].dope_vec.n = type_table[type_tx].dope_vec.upper_bound - type_table[type_tx].dope_vec.lower_bound;
        type_table[type_tx].size = type_table[type_tx].dope_vec.elem_type->size *            type_table[type_tx].dope_vec.n;
        type_table[type_tx].level = lev;
        for (ptrcd = &type_table[type_tx], ptrcd->dope_vec.const_offset = ptrcd->dope_vec.lower_bound; is_arr(ptrcd->dope_vec.elem_type); ptrcd = ptrcd->dope_vec.elem_type)
        {
            ptrcd->dope_vec.const_offset = ptrcd->dope_vec.const_offset * ptrcd->dope_vec.elem_type->dope_vec.n + ptrcd->dope_vec.elem_type->dope_vec.lower_bound;
        }
